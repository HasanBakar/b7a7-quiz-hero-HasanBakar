...
startQuiz.addEventListener("click", () => {
  ...
  let x = setInterval(() => {
    if (counterNum < 0) {
      countDown.classList.remove("flex"); // variable countDown spelling mistake resolved
      countDown.classList.add("hidden"); // variable countDown spelling mistake resolved
      counterNum = 3;
      count = 0;
      timer = null;
      quizData = null;
...
const loadQuiz = async () => {
  ...
  const data = await res.json(); // json is method so must a paranthsis. so it is resolved
  quizData = data;
  displayQuiz(data);
};
...
document.querySelector("#submit").addEventListener("click", () => { //addEventListener spelling mistake resoled
  if (answers.length < 6) {
    return;
  }
  quizTimer(true);
